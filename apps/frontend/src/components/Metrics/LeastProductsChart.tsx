...
export function LeastProductsChart({ data }: Props) {
  ...
  return (
    <Card className="border-slate-200 shadow-sm col-span-full">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-slate-700">Productos de Menor Rotación (Ingresos)</CardTitle>
      </CardHeader>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
            ...
            <Bar dataKey="total_revenue" radius={[0, 4, 4, 0]} barSize={35}>
              {data.map((_, index) => (
                <Cell key={`cell-${index}`} fill={index === 0 ? "#64748b" : "#fb923c"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
